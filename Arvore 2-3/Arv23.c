        ...
        if (folha(*Raiz)) {
            if ((**Raiz).nChaves == 1) {
                *Raiz = adicionaNo(*Raiz, info, NULL);
                ...
                // infoMeio é aproveitado nas funções quebraNo e criaNo
            ...
            // a info é menor que o valor do nó então ele vai para esquerda
            if (info < (**Raiz).chaveEsq)
                novo = insere23(*Raiz, &(**Raiz).esq, info, infoMeio);

            // a info é maior que o nó esquerdo e só tem um valor no nó, então ele vai para o centro
            else if ((**Raiz).nChaves == 1) 
                novo = insere23(*Raiz, &(**Raiz).centro, info, infoMeio);

            // a info é maior que o nó esquerdo e tem mais de um valor no nó, e ele é menor que o nó direto, então ele vai para o centro
            else if (info < (**Raiz).chaveDir) 
                novo = insere23(*Raiz, &(**Raiz).centro, info, infoMeio);
            
            // se chegar nesse else significa que a info é maior que o nó direito, então ele vai para a dereita
            else 
                novo = insere23(*Raiz, &(**Raiz).dir, info, infoMeio);

            // se novo for diferente de nulo, teve que quebrar o nó lá em cima, o novo vai para o nó pai
            if (novo != NULL) {
                // se o pai tiver só um valor o novo é inserido nele.
                if ((**Raiz).nChaves == 1) {
                    *Raiz = adicionaNo(*Raiz, *infoMeio, novo);
                    novo = NULL;
                } else {
                    // se não quebra o nó pai 
                    novo = quebraNo(Raiz, novo, *infoMeio, infoMeio);

                    // se o nó pai não tiver pai, 
                    if (pai == NULL) {
                        *Raiz = criaNO(*infoMeio, *Raiz, novo);
                        novo = NULL; 
                    }
